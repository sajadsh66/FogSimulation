...
class Link():
    ...
    def uplink(self):
        while True:
            ...
            msg.handling_time += self.propagation_delay
            if self.id == 1:
...
class IoT_Node():
    ...
    def response_handler(self):
        while True:
            ...
            # resp_time = msg.generation_time - self.env.now # it is not true!
            self.total_handling_time += msg.handling_time
            self.mean_resp_time = self.total_handling_time / self.total_handled_packets
            print(self.id, ":")
            print("Pscket {} handled in {} ms".format(self.total_handled_packets, msg.handling_time*1000))
            print("Mean response time till now = {} ms".format(self.mean_resp_time*1000))

    def send(self):
        while True:
            msg = (yield self.send_queue.get())
            yield self.env.timeout(((msg.size / self.I2F_link_rate)) * 0.001) # ms
            msg.handling_time += ((msg.size / self.I2F_link_rate)) * 0.001 # ms
            if self.id == "Inode1":
...
class Fog_Node():
    ...
    def send2fog(self):
        while True:
            ...
            msg.handling_time += ((msg.size / self.F2F_link_rate) * 0.001) # ms
            if msg.task_or_resp == "task":
                msg.nfw += 1
                msg.visited_fogs.append(self.id)
            if self.id == "Fnode1":
                linkFF.uplink_queue.put(msg)
                print("Packet {} of {} (type = {}) is offloading to Fnode2 (by Fnode1) in time = {}."
                      .format(msg.pkt_id, msg.src_id, msg.type, self.env.now))
            else:
                linkFF.downlink_queue.put(msg)
                print("Packet {} of {} (type = {}) is offloading to Fnode1 (by Fnode2) in time = {}."
                      .format(msg.pkt_id, msg.src_id, msg.type, self.env.now))
    ...
    def send2cloud(self):
        while True:
            ...
            msg.handling_time += ((msg.size / self.F2C_link_rate) * 0.001) # ms
            if msg.task_or_resp == "task":
                msg.nfw += 1
                msg.visited_fogs.append(self.id)
            if self.id == "Fnode1":
    ...
    def send2iot(self):
        while True:
            ...
            msg.handling_time += ((msg.size / self.F2I_link_rate) * 0.001) # ms
            if msg.task_or_resp == "task":
                msg.nfw += 1
                msg.visited_fogs.append(self.id)
            if self.id == "Fnode1":
