# Thresholds in Fog Nodes is removed
# Each Fog Node compare its estimated waiting time with neighbors' (estimated waiting time + 2*transmition delay + 2*propagation delay) to define best node for processing the task
...
class Packet(object):
    def __init__(self, pkt_id, generation_time=0, size=None, type=None, src_id=None, flow_id=0, nfw=0, fstc = False, task_or_resp="task",
                 AFP=True, NFP=False, LFP=False, fragmented=False, segment_number=None, TTL=0.2):
                ...
                if msg.fstc: # if force send to cloud flag is True, send it to cloud directly
                    ...
                    if len(self.connected_fogs)==0: ##############################################
                        ...
                        for i in self.connected_fogs:
                            neighbors.append([i, reachability_table.estimated_waiting_time[i] + 2*reachability_table.propagation_delay[self.id][i]
                    ...
                    if temp == 0 or (self.estimated_waiting_time + p_time) < temp: # process itself
...
if __name__ == '__main__':
    ...
    ### Topology of network should define here, because of all scenario should have same topology (for comparison) ###
        ...
        for r in range(1):
            ...
            for i in range(n_IoT):
                Inode.append(IoT_Node(env, id=i, pi=pi, pf=pf, pc=pc, bi=bi, light_packet_generation_rate=yi1, heavy_packet_generation_rate=yi2,
                                      AFP=AFP[r], NFP=NFP[r], LFP=LFP[r], round=z, TTL=waiting_threshold_time1[i]))
    ...
    # for z in range(ro):
    #     if len(results.AFP_L[z])==0:
    #         # print("AFP_L = 0")
    #         res[0].append(0)
    #     else:
    #         res[0].append(sum(results.AFP_L[z])/len(results.AFP_L[z]))
    ...
    #         res[1].append(sum(results.AFP_H[z]) / len(results.AFP_H[z]))
    #
    ...
    #         res[2].append(sum(results.LFP_L[z]) / len(results.LFP_L[z]))
    #
    ...
    #         res[3].append(sum(results.LFP_H[z]) / len(results.LFP_H[z]))
    #
    ...
    #         res[4].append(sum(results.NFP_L[z]) / len(results.NFP_L[z]))
    #
    ...
    #         res[5].append(sum(results.NFP_H[z]) / len(results.NFP_H[z]))
    #

    with open("enhance1.txt", "wb") as fp:  # Pickling
