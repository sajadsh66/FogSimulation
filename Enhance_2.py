# splitting tasks to sub-tasks in Fog Nodes
# splitting has memory overhead, but we assume that memory and buffers of nodes are unlimited
...
class Fog_Node():
    def __init__(self, env, id, n_forwarding_threshold=1):
        self.env = env
        self.process_queue = simpy.Store(env)
        ...
        while True:
            ...
            if msg.task_or_resp == "task":
                if msg.fstc: # if force send to cloud flag is True, send it to cloud directly
                    ...
                    if (self.estimated_waiting_time + p_time) < temp:  # process itself
                        ...
                        if msg.nfw >= self.forwarding_threshold or all_neighbors_visited:  # send to cloud
                            msg.fstc = True
                            # msg.nfw += 1
                            msg.visited_fogs.append(self.id)
                            for x in linkFC:
...
if __name__ == '__main__':
    ...
    for z in tqdm(range(ro)):
        for r in range(1):
            ...
            for i in range(n_Fog):
                Fnode.append(Fog_Node(env, id=i, n_forwarding_threshold=z+1))
    ...
    with open("enhance2.txt", "wb") as fp:  # Pickling
